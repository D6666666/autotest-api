...
class Email:
    ...
    def config_content(self):
        ...
        :return:
        """
        resultPath = self.log.get_report_path()
    ...
    def config_file(self):
        ...
        if self.check_file():
            ...
            # zippath = os.path.join(readConfig.proDir, "result", "Log+Html_result.zip")
            #
            # # zip file
            ...
            # filehtml['Content-Disposition'] = 'attachment; filename="Log+Html_result.zip"'
            ...
            # zip file
            files = glob.glob("%s/%s" % (reportpath,"*"))
            f = zipfile.ZipFile(zippath, 'w', zipfile.ZIP_DEFLATED)
            for file in files:
                # 修改压缩文件的目录结构
                f.write(file, '/report/'+os.path.basename(file))
